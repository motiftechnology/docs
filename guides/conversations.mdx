...
- Use the **inbox client** (`@chatafrika/inbox-client` or `packages/chatafrika-inbox-client` in the repo): pass your API URL and API keys, then call `listConversations(externalUserId)` and `getMessages(conversationId, externalUserId, options)`. In production, prefer `getHeaders()` from your backend so the secret is never exposed in the frontend.

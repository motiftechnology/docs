...
**Inbox & history:** To show "my conversations" and load past messages, use the same API keys:
- [List EXTERNAL conversations](/api-reference/conversations/list-external): `GET /conversations/external?external_user_id=...` — returns all EXTERNAL conversations where that user is a participant.
- [Get EXTERNAL messages](/api-reference/conversations/get-external-messages): `GET /conversations/external/:id/messages?external_user_id=...` — returns paginated message history. The SDK can expose `sdk.conversations.list()` and `convo.fetchMessages()` that call these (or your backend proxies them).
...
const conversation = await fetch('https://dev.chaterafrika.com/api/v1/conversations/external', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
...
If you prefer to use dashboard JWT instead of API keys, use `POST /conversations` with `Authorization: Bearer <access_token>` and body `{ type: "external", external_id, participants }`. See [Create Conversation](/api-reference/conversations/create).

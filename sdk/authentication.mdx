...
```bash
curl -X POST https://api.chaterafrika.com/api/v1/apps/YOUR_APP_ID/tokens \
...
app.post('/api/sdk-token', async (req, res) => {
  ...
  const r = await fetch(`https://api.chaterafrika.com/api/v1/apps/${process.env.APP_ID}/tokens`, {
    method: 'POST',
    headers: {
